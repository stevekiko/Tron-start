...
Windows 版本以 **Telegram Bot 为主要交互界面**，配合 `trongo.bat` 完成本地启停操作。推荐使用 [MSYS2](https://www.msys2.org/) 编译环境。
...
在 Windows 的**普通文件资源管理器**中，双击 `trongo.bat`，或在 CMD 里执行：
...
```
trongo.bat start    配置并启动 Telegram Bot 守护进程
trongo.bat stop     停止后台引擎进程
trongo.bat -r       查看爆号结果文件
trongo.bat -s       查看当前算力
```
